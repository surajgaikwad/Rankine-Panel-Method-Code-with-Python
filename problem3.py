...
while True:
    if local_to_global=="y":
        ...
        xifloat=float(xistr)
        etafloat=float(etastr)
        #X=X(xi,eta)
        #Y=Y(xi,eta)
        gp=[xifloat,etafloat]
        H=[X(xifloat,etafloat),Y(xifloat,etafloat)]
        print('\nlocal coordinates',gp,'<-->' ,H, 'in global coordinates for Quadrilateral PQRS')
        local_to_global=input('\nto find global coordinates from local coordinates enter "y" / to finish enter "n":')
    else :
        break

xi=[xx1,xx2,xx3,xx4,xx1]
eta=[yy1,yy2,yy3,yy4,yy1]

def s(N,n):
    #n=n-1
    if N==1:
        s=((eta[n+1]-eta[n])*(((G[0]-xi[n])**2)+((G[2])**2))-((xi[n+1]-xi[n])*(G[0]-xi[n])*(G[1]-eta[n])))
        #print("s1:",s)
    else :
        s=((eta[n+1]-eta[n])*(((G[0]-xi[n+1])**2)+((G[2])**2))-((xi[n+1]-xi[n])*(G[0]-xi[n+1])*(G[1]-eta[n+1])))
        #print("s2:",s)
    return s

def R(n):
    #n=n-1
    R=(((G[0]-xi[n])**2)+((G[1]-eta[n])**2)+((G[2])**2))**0.5
    #print("R:",R)
    return R
...
def c(N,n):
    #n=n-1
    if N==1:
        c=(R(n)*G[2]*(xi[n+1]-xi[n]))
        #print("c1:",c)
    else :
        c=(R(n+1)*G[2]*(xi[n+1]-xi[n]))
        #print("c2:",c)
    return c

def S3(n):
    #n=n-1
    S3=(s(1,n)*c(2,n))-(s(2,n)*c(1,n))
    #print('S3:',S3)
    return S3

def C3(n):
    #n=n-1
    C3=(c(1,n)*c(2,n))+(s(1,n)*s(2,n))
    #print("C3:",C3)
    return C3

def sine(n):
    #n=n-1
    sine=(eta[n+1]-eta[n])/(((xi[n]-xi[n+1])**2+(eta[n]-eta[n+1])**2)**0.5)
    #print("sine:",sine)
    return sine

def cosine(n):
    #n=n-1
    cosine=(xi[n+1]-xi[n])/(((xi[n]-xi[n+1])**2+(eta[n]-eta[n+1])**2)**0.5)
    #print("cosine:",cosine)
    return cosine

def S(n):
    S=((xi[n]-xi[n+1])**2+(eta[n]-eta[n+1])**2)**0.5
    #print("S:",S)
    return S
...
def psi1(n):
    ...
    n=0
    while n<4:
        psi1=(((G[0]-xi[n])*sine(n))-((G[1]-eta[n])*cosine(n)))*(math.log(abs((R(n)+R(n+1)+S(n))/(R(n)+R(n+1)-S(n)))))
        sum=sum+psi1
        #print("n:",n," psi1sum:",sum)
        n=n+1
    return sum
